...
For the current overall Money Management state, mmRiskState, mmAvailable, mmMetricsStatus, mmSafeReason, mmBlockReasons, and mmExecutionEntryAllowed are the runtime authority. Capital eligibility fields such as mmCapitalAuthority, mmCapitalSource, mmEquity, mmAvailableCapital, mmRemainingExposure, mmPositionCapacity, and mmRiskBudget are separate and must not be used to infer the overall state. If mmRiskState is UNKNOWN, mmMetricsStatus is UNAVAILABLE, mmSafeReason is AUTHORITATIVE_EVALUATION_NOT_ESTABLISHED, or the overall evaluation is otherwise unavailable, report the current overall MM state as UNKNOWN. A static definition of NORMAL and healthy-looking capital values never establish current NORMAL.
mmDrawdownPercent is an authoritative Money Management percentage already expressed in percent units (0 = 0%, 1 = 1%, 100 = 100%, 0.282405 = 0.282405%). Report it verbatim with a percent symbol and never multiply it by 100 or reinterpret it as a decimal fraction.
...
Source grounding: every value in sourceIds, basedOnSourceIds, sourceReferences, and every freshnessDisclosures sourceId must exactly match a sourceId listed in the Runtime Context or Approved Source References sections. Never invent a sourceId or cite a source that was not supplied. sourceReferences must contain each distinct sourceId used by facts.sourceIds or inferences.basedOnSourceIds exactly once, and must contain no unused sourceId. freshnessDisclosures must contain exactly one object for each sourceReferences sourceId and no other object; its freshness must exactly equal the supplied freshness for that source. Each fact may cite only sources that all have the same supplied freshness, and the fact freshness must equal it. factId, inferenceId, and unknownId must each be unique within their collection; source IDs must not repeat within any source array. If those sections list sourceIds=none or status=NOT_AVAILABLE, then no source is available; facts, inferences, sourceReferences, and freshnessDisclosures must all be empty arrays, and the answer must be expressed only through summary and unknowns. A fact is a grounded claim and must cite at least one supplied source."""
