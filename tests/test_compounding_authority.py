...
def test_unsaved_compounding_draft_cannot_affect_runtime_sizing():
    saved = config(False)
    unsaved_draft = {"compoundingEnabled": True}
    result = authority(saved, D("1200"))
    assert unsaved_draft["compoundingEnabled"] is True
    assert saved.compounding_enabled is False
    assert result.capital_basis == D("1000")
    assert result.risk_budget == D("5")
